...
class MyGame:
    ...
    def game_over1(self):  # 游戏结束（AIGC）
        while True:
            self.info = pygame.display.Info()
            x, y = self.info.current_w / 2, self.info.current_h / 2
            font = pygame.font.SysFont('华文行楷', 75)
            text = font.render("恭喜游戏结束", True, (255, 0, 0))
            self.screen.blit(text, (x - text.get_width() / 2, (y - text.get_height() / 2) / 2))
            pygame.display.update()
    ...
    def game_over2(self):  # 游戏结束（AIGC）
        while True:
            self.info = pygame.display.Info()
            x, y = self.info.current_w / 2, self.info.current_h / 2
            font = pygame.font.SysFont('华文行楷', 55)
            text = font.render("倒计时结束，游戏失败", True, (255, 0, 0))
            self.screen.blit(text, (x - text.get_width() / 2, (y - text.get_height() / 2) / 2))
            pygame.display.update()
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
    ...
    def game1(self):
        while True:
            ...
            self.e_time = pygame.time.get_ticks()  # 倒计时组件（AIGC）
            if self.e_time - self.s_time >= 1000:
                self.countdown -= 1
    ...
    def draw_countdown(self):  # 倒计时组件（AIGC）
        ...
        text = font.render(f"倒计时: {self.countdown}秒", True, (255, 0, 0))
